...
def _get_trans_dir() -> str:
    """
    Return the directory used for transcriptions.
    Allows tests to override TRANSCRIPTIONS_DIR via the api module alias.
    """
    try:
        api_mod = importlib.import_module('backend.routes.api')
        return getattr(api_mod, 'TRANSCRIPTIONS_DIR', _config.TRANSCRIPTIONS_DIR)
    except Exception:
        return _config.TRANSCRIPTIONS_DIR
...
async def delete_trans(filename: str, client_id: str) -> Dict[str, str]:
    ...
    for ext in (".wav", ".mp3"):
        audio_path = _safe_join(_get_trans_dir(), audio_dir, audio_name_base + ext)
...
async def get_trans(filename: str, client_id: str) -> FileResponse:
    p = _safe_join(_get_trans_dir(), client_id, filename)
...
async def view_transcription(client_id: str, filename: str, req: Request) -> Response:
    ...
    if os.path.isfile(cleaned_path):
        ...
        temp_path = _safe_join(_get_trans_dir(), "live_audio", filename)
