...
def setup_isolation_env() -> Any:
    ...
    old_transcriptions_dir = api_mod.TRANSCRIPTIONS_DIR
    ...
    with open(os.path.join(user_a_dir, "file_a.txt"), "w") as f: 
        f.write("content a")
    ...
    with open(os.path.join(batch_audio_dir, audio_a), "wb") as f: 
        f.write(b"audio a content")
    ...
    with open(os.path.join(live_audio_dir, live_txt_a), "w") as f: 
        f.write("live content a")
    ...
    api_mod.TRANSCRIPTIONS_DIR = old_transcriptions_dir
    config_mod.TRANSCRIPTIONS_DIR = old_transcriptions_dir

def test_isolation_list(setup_isolation_env: Dict[str, str], client: TestClient) -> None:
...
def test_isolation_download_transcript(setup_isolation_env: Dict[str, str], client: TestClient) -> None:
    env = setup_isolation_env
...
def test_isolation_download_audio(setup_isolation_env: Dict[str, str], client: TestClient) -> None:
    env = setup_isolation_env
...
def test_isolation_view_live_temp(setup_isolation_env: Dict[str, str], client: TestClient) -> None:
    env = setup_isolation_env
    # User B tries to view User A's temporary live transcript
    resp = client.get(f"/view/{env['user_b']}/{env['live_txt_a']}")
    assert resp.status_code == 403

def test_websocket_forbidden(setup_isolation_env: Dict[str, str], client: TestClient) -> None:
...
def test_batch_chunk_forbidden(setup_isolation_env: Dict[str, str], client: TestClient) -> None:
