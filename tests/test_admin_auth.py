...
class TestChangeModelAuth:
    ...
    def test_change_model_no_env_key_is_accessible(self, monkeypatch: pytest.MonkeyPatch, client: TestClient) -> None:
    ...
    def test_change_model_wrong_key_returns_403(self, monkeypatch: pytest.MonkeyPatch, client: TestClient) -> None:
        monkeypatch.setenv("ADMIN_API_KEY", _VALID_KEY)
        resp = client.post("/change_model?model=mock", headers={"X-Admin-Key": "mauvaise"})
        assert resp.status_code == 403

    def test_change_model_no_key_header_returns_403(self, monkeypatch: pytest.MonkeyPatch, client: TestClient) -> None:
        monkeypatch.setenv("ADMIN_API_KEY", _VALID_KEY)
        resp = client.post("/change_model?model=mock")
        assert resp.status_code == 403

    def test_change_model_valid_key_returns_200(self, monkeypatch: pytest.MonkeyPatch, client: TestClient) -> None:
        monkeypatch.setenv("ADMIN_API_KEY", _VALID_KEY)
...
class TestGitUpdateAuth:
    ...
    def test_git_update_wrong_key_returns_403(self, monkeypatch: pytest.MonkeyPatch, client: TestClient) -> None:
        monkeypatch.setenv("ADMIN_API_KEY", _VALID_KEY)
        resp = client.post("/git_update", headers={"X-Admin-Key": "wrong"})
        assert resp.status_code == 403

    def test_git_update_missing_header_returns_403(self, monkeypatch: pytest.MonkeyPatch, client: TestClient) -> None:
        monkeypatch.setenv("ADMIN_API_KEY", _VALID_KEY)
        resp = client.post("/git_update")
        assert resp.status_code == 403
