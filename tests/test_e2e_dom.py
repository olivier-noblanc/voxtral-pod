...
def test_all_js_ids_exist_in_rendered_html(page: tuple) -> None:
    """Chaque ID référencé dans app.js doit exister dans le DOM rendu."""
    p, base_url = page
    p.goto(base_url)

    missing = [id_ for id_ in _get_js_ids() if p.locator(f"#{id_}").count() == 0]
...
def test_albertoptions_visible_when_albert_selected(page: tuple) -> None:
    """
    ...
    selector = p.locator("#modelSelector")
    if selector.count() == 0:
    ...
    options_panel = p.locator("#albertOptions")
    assert options_panel.count() == 1, "#albertOptions absent du DOM"
    expect(options_panel).to_be_visible(timeout=2000)


def test_albertpartial_unchecked_when_albert_selected(page: tuple) -> None:
    """
    ...
    checkbox = p.locator("#albertPartial")
...
def test_websocket_partial_false_by_default(page: tuple) -> None:
    """
