...
def test_download_live_audio(setup_test_files: Dict[str, Any], client: TestClient) -> None:
...
def test_download_batch_audio(setup_test_files: Dict[str, Any], client: TestClient) -> None:
    data = setup_test_files
    resp = client.get(f"/download_audio/{data['client_id']}/{data['batch_audio']}")
    assert resp.status_code == 200
    assert resp.headers["content-type"] == "audio/wav"

def test_download_transcript(setup_test_files: Dict[str, Any], client: TestClient) -> None:
    data = setup_test_files
